...
struct Options {
    ...
    init(arguments: [String]) throws {
        ...
        self.role = role
        self.name = Self.value(after: "--name", in: arguments) ?? "Syphon26 App-To-App Benchmark"
        ...
        self.stateURL = URL(fileURLWithPath: Self.requiredValue(after: "--state", in: arguments))
...
func runServer(options: Options) throws {
    guard let device = MTLCreateSystemDefaultDevice(),
        ...
        throw Syphon26Error.metal(
            Syphon26RuntimeIssue(operation: "createAppToAppServerDevice", reason: "Metal device or command queue is unavailable")
        )
    }

    try? FileManager.default.removeItem(at: options.stateURL)
    ...
    let streamDescription = try Syphon26StreamDescription(
        streamID: Syphon26StreamID.unchecked("app-to-app-\(options.name)"),
        name: options.name,
        appName: "Syphon26AppToAppBenchmarkServer",
        width: options.width,
        height: options.height,
        pixelFormat: options.pixelFormat,
        controlPlaneServiceName: Syphon26.defaultControlPlaneServiceName
    )
    let controlPlane = Syphon26FileControlPlane(stateURL: options.stateURL)
    ...
    let summary = ServerSummary(
        ...
        statePath: options.stateURL.path
    )
...
func runClient(options: Options) throws {
    guard let device = MTLCreateSystemDefaultDevice() else {
        throw Syphon26Error.metal(
            Syphon26RuntimeIssue(operation: "createAppToAppClientDevice", reason: "Metal device is unavailable")
        )
    }
...
func runServerPhase(
    ...
    controlPlane: Syphon26FileControlPlane,
...
func runClientPhase(
    ...
    while nowSeconds() - start < durationSeconds {
        guard let metadata = try? controlPlane.metadata() else {
            usleep(idlePoll)
            continue
        }
...
func encodeClear(into texture: any MTLTexture, commandQueue: any MTLCommandQueue, sequence: UInt64) throws {
    guard let commandBuffer = commandQueue.makeCommandBuffer() else {
        throw Syphon26Error.metal(
            Syphon26RuntimeIssue(operation: "createAppToAppCommandBuffer", reason: "Metal returned nil command buffer")
        )
    }
    let descriptor = MTLRenderPassDescriptor()
    guard let colorAttachment = descriptor.colorAttachments[0] else {
        throw Syphon26Error.metal(
            Syphon26RuntimeIssue(operation: "createAppToAppColorAttachment", reason: "Metal returned nil color attachment")
        )
    }
    ...
    guard let encoder = commandBuffer.makeRenderCommandEncoder(descriptor: descriptor) else {
        throw Syphon26Error.metal(
            Syphon26RuntimeIssue(operation: "createAppToAppRenderEncoder", reason: "Metal returned nil render encoder")
        )
    }
    encoder.endEncoding()
