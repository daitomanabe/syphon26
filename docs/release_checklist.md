...
- `scripts/run_test_pattern_pair.sh --duration 1 --fps 60 --width 1280 --height 720 --orientation normal` proves the production-XPC test-pattern server/client pair with expected frame-count gates.
- `scripts/run_test_pattern_pair.sh --duration 0.5 --fps 30 --width 640 --height 360 --orientation flipY` and `--orientation rotate180` prove the alternate orientation modes.
